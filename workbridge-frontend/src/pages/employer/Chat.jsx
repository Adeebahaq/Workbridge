        ...
        <div className="flex-1 overflow-y-auto">
          {chatJobs.length===0&&(
            <div className="flex flex-col items-center justify-center py-12 px-4 text-slate-400 text-center">
              ...
              <p className="text-xs mt-1">Hire a worker to start chatting</p>
            </div>
          )}
          {chatJobs.map(j=>{
            const wname=getWorkerName(j);
            const winit=getInitials(wname);
            const isActive=j._id===activeJob;
            const lastMsg=messages.length>0&&j._id===activeJob?messages[messages.length-1]:null;
            return (
              <button key={j._id} onClick={()=>{setActiveJob(j._id);navigate(`/employer/chat/${j._id}`);}}
                ...
                <div className="relative">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center font-black text-xs shrink-0 ${isActive?"bg-teal-500 text-white":"bg-slate-200 text-slate-600"}`}>{winit}</div>
              ...
              return (
                <div key={i} className={`flex ${isMine?"justify-end":"justify-start"}`}>
                  ...
                  <div className={`max-w-xs lg:max-w-sm px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${isMine?"bg-[#0F172A] text-white rounded-br-sm":"bg-white shadow-sm text-slate-800 border border-slate-100 rounded-bl-sm"}`}>
                    {m.text}
                    <p className={`text-[10px] mt-1 ${isMine?"text-slate-400":"text-slate-400"}`}>{fmtTime(m.createdAt)}</p>
                  </div>
                </div>
              );
            })}
