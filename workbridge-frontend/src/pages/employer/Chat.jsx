...
export default function Chat() {
  ...
  const sendVoiceMessage = async () => {
    ...
    try {
      ...
      sendVoice(String(workerId), fullUrl, duration);
      setAudioBlob(null);
      setAudioURL(null);
  ...
  const getInitials = (name = "") =>
    name.split(" ").slice(0, 2).map(p => p[0]).join("").toUpperCase() || "W";
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {uniqueChatJobs.map(j => {
            ...
            return (
              <button
                key={j._id}
                onClick={() => { setActiveJob(j._id); navigate(`/employer/chat/${j._id}`); }}
