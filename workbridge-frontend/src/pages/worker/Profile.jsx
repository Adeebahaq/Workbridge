...
function EditField({ label, name, value, onChange, half = false, type = "text", readOnly = false }) {
  return (
    <div className={half ? "col-span-1" : "col-span-2"}>
      <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">
        {label}
    ...
    <div className="max-w-3xl mx-auto px-4 py-6">
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <div className="flex items-center gap-2 justify-center sm:justify-start mt-2 flex-wrap">
              ...
              {profile.status === "Active" && (
                <span className="px-3 py-1 rounded-full text-xs font-bold bg-teal-100 text-teal-700">✓ CNIC Verified</span>
              )}
            </div>
          ...
          <div className="shrink-0">
            {editing ? (
              ...
              <button
                ...
                className="border border-slate-200 text-slate-700 px-4 py-2 rounded-xl text-sm font-semibold hover:bg-slate-50"
              >
                ✏️ Edit Profile
              </button>
            )}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field
            label="Services"
            value={Array.isArray(profile.services)
              ? profile.services.map(s => s?.name || s).join(", ")
              : "—"}
          />
        </div>
      </div>
