          ...
          {activeTab === "info" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                <div key={label} className={label === "Current Address" ? "col-span-1 sm:col-span-2" : ""}>
...
function PendingRow({ worker, onReview }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 bg-white rounded-2xl border border-slate-100 shadow-sm p-4 hover:border-amber-200 hover:shadow-md transition-all">
      ...
      <button onClick={() => onReview(worker)}
        className="w-full sm:w-auto shrink-0 flex items-center justify-center gap-1 bg-[#0F172A] hover:bg-slate-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition-all">
        Review <ChevronRight size={12} />
      </button>
    </div>
  );
...
export default function AdminDashboard() {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 mt-14">
      ...
      <aside className="w-56 shrink-0 bg-[#0F172A] flex-col min-h-screen fixed left-0 top-0 z-30 hidden md:flex">
      ...
      {/* ── Mobile Bottom Nav ── */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-[#0F172A] border-t border-slate-800 flex">
        {TABS.map(({ id, label, Icon }) => (
          <button key={id} onClick={() => setTab(id)}
            className={`flex-1 flex flex-col items-center gap-1 py-2.5 text-[10px] font-bold transition-all relative ${
              tab === id ? "text-teal-400" : "text-slate-500"
            }`}>
            <Icon size={18} />
            {label}
            {id === "verify" && pending.length > 0 && (
              <span className="absolute top-1.5 right-1/4 text-[9px] font-black bg-amber-400 text-amber-900 px-1 rounded-full">
                {pending.length}
              </span>
            )}
          </button>
      ...
      {/* ── Main Content ── */}
      <main className="flex-1 ml-0 md:ml-56 p-4 md:p-6 min-h-screen pb-20 md:pb-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <div className="relative w-full sm:w-auto">
                ...
                <input
                  ...
                  className="border border-slate-200 rounded-xl pl-8 pr-3 py-2 text-sm text-slate-700 focus:outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-100 w-full sm:w-52"
                />
              </div>
            </div>

            <div className="flex gap-1 bg-white rounded-2xl p-1 shadow-sm border border-slate-100 w-full sm:w-fit overflow-x-auto">
              {[
                { val: "",                     label: "All" },
